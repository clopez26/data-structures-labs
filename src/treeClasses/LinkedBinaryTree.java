...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	public LinkedBinaryTree<E> clone() throws CloneNotSupportedException { 
		LinkedBinaryTree<E> clone = new LinkedBinaryTree<>(); 

		if (!isEmpty())
			clone.addRoot(root().getElement()); 
		cloneSubtree(root(), clone, clone.root()); 

		return clone; 
	}

	private void cloneSubtree(Position<E> pos, LinkedBinaryTree<E> t, Position<E> pos2) {
		for (Position<E> pos1 : children(pos)) {

			Position<E> parent = this.parent(pos1);
			Position<E> pos_2 = null;

			if(this.hasLeft(parent) && this.left(parent).equals(pos1))
				pos_2 = t.addLeft(pos2, pos1.getElement());
					else
						pos_2 = t.addRight(pos2, pos1.getElement());
